...
for emotion, df_emotion in emotion_dfs.items():
    ...
    tfidf_data[emotion] = vectorizer.transform(texts)
...
for emotion in emotion_dfs.keys():
    ...
    with open(f'../models/best_model_tfidf_{emotion}.pkl', 'wb') as handle:
        pickle.dump(best_model, handle, protocol=pickle.HIGHEST_PROTOCOL)

    # Save evaluation report
    with open(f'../results/evaluation_reports/tfidf_{emotion}_report.txt', 'w') as f:
    ...
    plt.savefig(f'../results/figures/tfidf_{emotion}_confusion_matrix.png')
    plt.close()
    ...
    plt.savefig(f'../results/figures/tfidf_{emotion}_roc_curve.png')
    plt.close()

# Save the metrics to CSV files
for model_name, metrics in model_metrics.items():
    df_metrics = pd.DataFrame(metrics, columns=['Emotion', 'Accuracy', 'AUC', 'F1', 'Recall', 'Precision'])
    df_metrics.to_csv(f'../results/evaluation_reports/tfidf_{model_name}_metrics.csv', index=False)
...
plt.title('Model Performance vs Emotion (TF-IDF)')
plt.tight_layout()
plt.savefig('../results/figures/model_performance_vs_emotion_tfidf.png')
plt.close()
...
plt.xlabel('AUC')
plt.ylabel('Model-Emotion')
plt.title('Model AUC vs Emotion (TF-IDF)')
plt.tight_layout()
plt.savefig('../results/figures/model_auc_vs_emotion_tfidf.png')
plt.close()

# Save the performance data to a CSV file
df_performance.to_csv('../results/model_performance_tfidf.csv', index=False)

# Print the overall performance summary
print("Model Performance Summary (TF-IDF):")
